...
public class StringTokenizer {
	...
	public String getNextTokensAndSkip(String textSkip) {
		...
		while (hasNextMoreTokens()) sb.append(nextToken());
		return new Strings(sb.toString()).skip(textSkip).toString();
	}
	...
	public String getPreviouTokensAndSkip(String skipText) {
		...
		return new Strings(sb.toString()).skip(skipText).toString();
	}
	...
	public String getAllTokensDelimiter() {
		...
		if (delimiter.length() > 1) {
			...
			for (String part : parts) sb.append(test(part)).append("|");
			if(sb.length() > 2)
				sb.delete(sb.length() - 1, sb.length());
	...
	public String getTokenDelimiter() {
		String delimiters = delimiterTokens[Math.min(currentPosition, delimiterTokens.length)];
	...
	public StringTokenizer addDelimiter(String delimiter) {
		...
		if(delimiter.length() > 1) {
			...
			for (String delimiterPart : delimiterParts) sb.append(test(delimiterPart)).append("|");
			String[] delimiterParts2 = test(getTokenDelimiter()).split("");
			for (String s : delimiterParts2) sb.append(test(s)).append("|");
			if(sb.length() > 2)
				sb.delete(sb.length() - 1, sb.length());
		} else {
		...
		StringBuilder sb2 = new StringBuilder();
		sb2.append(str);
		tokens = sb2.toString().split(sb.toString());
		return this;
	}
	...
	public StringTokenizer setDelimiter(String delimiter) {
		...
		if(delimiter.length() > 1) {
			String[] delimiterParts = (delimiter.replace("|", "\\|")).split("");
			for (String delimiterPart : delimiterParts) sb.append(delimiterPart.replace("*", "\\*")).append("|");
			if(sb.length() > 2) sb.delete(sb.length() - 1, sb.length());
		} else sb.append(delimiter);
		StringBuilder sb2 = new StringBuilder();
		sb2.append(str);
